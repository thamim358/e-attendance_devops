...
const ICAModuleGroupModal = ({ visible, onCancel, storeStudent }) => {
  const { StudentModuleByID } = useSelector((state) => state?.studentState);

  return (
    <Modal
      title={<p className="font-sans font-semibold text-xl">International student module groups </p>}
      visible={visible}
      onCancel={onCancel}
      centered={true}
      footer={null}
      ...
      <div className="font-sans mt-12">
        <div>
          <p className="font-sans text-base">
            <strong>Student Name: </strong> {storeStudent}
            {StudentModuleByID?.length > 0 && ` (${StudentModuleByID[0]?.Student_ID})`}
          </p>
        </div>
...
export default ICAModuleGroupModal;
